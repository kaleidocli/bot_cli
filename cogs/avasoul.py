    ...
    async def land(self, ctx, *args):
        ...
        try:
            if raw[0] == 'create':
                ...
                await self.client._cursor.execute(f"""INSERT INTO pi_land VALUES (
                    {latest[0]+1}, 
                    'land.{latest[0]+1}', 
                    '{ctx.author.id}', 
                    '{cur_PLACE}', 
                    ...
                    'alre', 
                    '', 
                    ''); UPDATE personal_info SET money=money-{money_lim} WHERE id='{ctx.author.id}'; INSERT INTO pi_tax VALUES ('land.{latest[0]+1}', 50, 50, 50, 50)""")
