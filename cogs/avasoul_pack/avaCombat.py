...
# ================== COMBAT ==================
    ...
    # [lock], [effect], [distance]
    #
    # <!> distance is only init in range and melee_PVE
    #
    # <!> CONCEPTS
    # ---- WEAPON ----
    ...
    async def range(self, ctx, *args):
        ...
        if __mode == 'PVP': 
            ...
            try: t_Ax, t_Ay, t_Bx, t_By, t_name, t_AFlame, t_AIce, t_ADark, t_AHoly, t_speed, t_DEFMA, t_DEFPY, t_mob_code = await self.client.quefe(f"SELECT limit_Ax, limit_Ay, limit_Bx, limit_By, name, au_FLAME, au_ICE, au_DARK, au_HOLY, speed, defense_magic, defense_physic, mob_code FROM environ_MOB WHERE mob_id='{target_id}';")
            # E: Invalid target, or target's not in the same region
            ...
            protoMob = self.client.DBC['model_mob'][t_mob_code]
        ...
        if __mode == 'PVP':
            ...
            if not int(CE['distance']): CE['distance'] = self.ceDistanceGenerator(protoMob.distance_min, protoMob.distance_max)                                 # Distance are generated by the info in DBC['model_mob']
            distance = int(CE['distance'])
...
# ================== MODULES ==================
    ...
    async def PVE_melee(self, MSG, target_id, raw_move, CE=None, CE_ttl=0):
        ...
        # GET Mob info =======================
        try:
            t_mob_code, t_name, t_speed, t_str, t_chain, t_lp, t_illulink, t_effect, t_lockon_max, t_defpy, t_branch = await self.client.quefe(f"SELECT mob_code, name, speed, str, chain, LP, illulink, effect, lockon_max, defense_physic, branch FROM environ_mob WHERE mob_id='{target_id}' AND region='{cur_PLACE}' AND {cur_X} > limit_Ax AND {cur_Y} > limit_Ay AND {cur_X} < limit_Bx AND {cur_Y} < limit_By;")
        # If mob not found (either mistyping or outdated lock), set lock to 'n/a'
            ...
            try:
                ...
                target_id = target_id[0]
                t_mob_code, t_name, t_speed, t_str, t_chain, t_lp, t_illulink, t_effect, t_defpy, t_branch = await self.client.quefe(f"SELECT mob_code, name, speed, str, chain, LP, illulink, effect, defense_physic, branch FROM environ_mob WHERE mob_id='{target_id}' AND region='{cur_PLACE}' AND {cur_X} > limit_Ax AND {cur_Y} > limit_Ay AND {cur_X} < limit_Bx AND {cur_Y} < limit_By;")
                CE['lock'] = target_id
            except IndexError:
        ...
        protoMob = self.client.DBC['model_mob'][t_mob_code]
        if CE:
            if not int(CE['distance']):
                CE['distance'] = self.ceDistanceGenerator(protoMob.distance_min, protoMob.distance_max)                                 # Distance are generated by the info in DBC['model_mob']
        ...
        async def battle():
            ...
            # Calc dmg
            ttl_plus = 10
            dmg, dmg_q, m_art, t_bonus, icon_sequence, ttl_plus = await dmg_calc(raw_move, ttl_plus=ttl_plus)
            ...
            if isUserOnLock:
                ...
                if 'evade' in bonus:
                    await MSG.channel.send(embed=discord.Embed(title=f"<:evading:615285957889097738> **{MSG.author.mention}** evaded!", color=0xF95656))
                if 'evade' in t_bonus:
                    await MSG.channel.send(embed=discord.Embed(title=f"\n<:evading:615285957889097738> **{t_name}** evaded!", color=0xF95656))
                targetPack = (target_id, t_name, t_dmg, t_icon_sequence, t_illulink)
            else:
                targetPack = (target_id, t_name, 0, '----- ignoring -----', t_illulink)
...
# ================== MODULES 2 ==================
    ...
    async def mobLooting(self, mob_pack, user_pack, mobProto, user_mention='adventurer'):
        ...
        _, rewards, reward_query, mob_name, mob_branch = await self.client.quefe(f"SELECT mob_code, rewards, reward_query, name, branch FROM environ_mob WHERE mob_id='{mob_id}';")

        # Merit calc            1 mob's EVO = 9 user's EVO
        merrire = 9 * (mobProto.evo+20 - user_evo)
        ...
        try:
            ...
            if len(mems) > 1:
                ...
                return f"<:chest:507096413411213312> Congrats **{user_mention}**, you and {len(mems) - 1} other party members had killed **「`{mob_id}` | {mob_name}」**!{rewards}"
            else:
                await self.client._cursor.execute(reward_query.replace('user_id_here', str(user_id)))
                return f"<:chest:507096413411213312> Congrats **{user_mention}**, you've finished **「`{mob_id}` | {mob_name}」**!{rewards}"
        except:
            await self.client._cursor.execute(reward_query.replace('user_id_here', str(user_id)))
            return f"<:chest:507096413411213312> Congrats **{user_mention}**, you've finished **「`{mob_id}` | {mob_name}」**!{rewards}"
    ...
    async def mobResupply(self, mob_id, mobProto):
        ...
        for stuff in mobProto.rewards:
            # Gacha
            if await self.utils.percenter(int(stuff[2]), total=100):
                formattedReward = self.utils.rewardToQueryAndName(stuff)
                # Item
                if formattedReward[0]:
                    objecto.append(formattedReward[0])
                # Status
                if formattedReward[1]:
                    status.append(formattedReward[1])
                # Name
        ...
        if status:
            reward_query += f"""UPDATE personal_info SET {', '.join(status)} WHERE id="user_id_here";"""
    ...
    async def battleInfoFormatting(self, statusPack, playerPack, targetPack):
        ...
        temb = discord.Embed(description=f""">>> :innocent: **:::** {playerPack[2]}\n:smiling_imp: **:::** {targetPack[3]}""", color=0xC6FF54)
        # FIELD: Player (Left)
    ...
    def ceDistanceGenerator(self, distance_min, distance_max):
        return random.choice(range(distance_min, distance_max))
